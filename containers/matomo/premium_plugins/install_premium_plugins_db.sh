...
usage() {
    echo "Usage: $0 -i|--rollup-id <integer> -u|--db-user <string> -l|--db-password <string> -d|--database <string>" 1>&2; exit 1;
}

OPTIONS=$(getopt -n "$0" -o i:u:l:d: --long "rollup-id:,db-user:,db-password:,database:"  -- "$@")
