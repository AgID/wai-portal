...
@section('content')
  <form class="Form Form--spaced u-text-r-xs" method="post" action="{{ route('admin-do_password_reset') }}">
      ...
      <div class="Form-field {{ $errors->has('token') ? 'is-invalid' : '' }}">
        @if ($errors->has('token'))
          <div class="Alert Alert--error Alert--withBg u-padding-r-top u-padding-r-bottom u-padding-r-right">
            ...
            @endif
            <label class="Form-label is-required" for="token">Codice{{-- //TODO: put message in lang file --}}</label>
            <input class="Form-input" id="token" name="token" type="token" aria-required="true" value="{{ old('token') }}" required>
          @if ($errors->has('token'))</div>@endif
      </div>
      @endif
    </fieldset>
    <fieldset class="Form-fieldset">
      <div class="Form-field {{ $errors->has('password') ? 'is-invalid' : '' }}">
        @if ($errors->has('password'))
          <div class="Alert Alert--error Alert--withBg u-padding-r-top u-padding-r-bottom u-padding-r-right">
            ...
            @endif
            <label class="Form-label is-required" for="password">Nuova password{{-- //TODO: put message in lang file --}}</label>
            <input class="Form-input" id="password" name="password" type="password" aria-required="true" required>
            <p class="Form-message">Deve essere lunga almeno 8 caratteri e contenere maiuscole, minuscole, numeri e simboli.{{-- //TODO: put message in lang file --}}</p>
          @if ($errors->has('password'))</div>@endif
      </div>
      <div class="Form-field {{ $errors->has('password') ? 'is-invalid' : '' }}">
        @if ($errors->has('password'))
          <div class="Alert Alert--error Alert--withBg u-padding-r-top u-padding-r-bottom u-padding-r-right">
            ...
            @endif
            <label class="Form-label is-required" for="password_confirmation">Conferma password{{-- //TODO: put message in lang file --}}</label>
    ...
    <div class="Form-field Grid-cell u-textCenter">
      <button type="submit" class="Button Button--default u-text-xs submit">Reimposta password{{-- //TODO: put message in lang file --}}</button>
    </div>
  </form>
@endsection
