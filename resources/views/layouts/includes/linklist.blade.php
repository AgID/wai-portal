...
@foreach ($items as $item)
    ...
    <li data-megamenu-class="Megamenu-item @if ($item['url'] == url(request()->path())) is-current @endif">
        <a href="{{ $item['url'] }}" class="@if ($item['url'] == url(request()->path())) is-current u-textWeight-700 @endif Linklist-link" data-megamenu-class="@if ($item['url'] == url(request()->path())) is-current @endif">
